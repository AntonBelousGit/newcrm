...
@section('content')
    ...
    <form class="form-horizontal" action="{{route('admin.tracker.update', $id)}}" id="kt_form_1" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <select  name="order_id" class="form-control ">
                                @foreach($orders as $item)
                                    <option value="{{$item->id}}" @if($item->id == $id ) selected @endif>{{$item->id}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
